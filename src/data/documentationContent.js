...
export const documentationSections = [
  ...
  {
    id: 'collecte-assistee-ia',
    titre: 'Collecte assistée par IA',
    icon: 'smart_toy',
    blocs: [
      { type: 'p', texte: 'La collecte assistée par IA aide à structurer des informations déjà disponibles. Open Carbon Tool ne contacte aucun LLM : le fichier reste local et vous le transmettez vous-même, hors de l’application, à un service autorisé par votre organisation.' },
      { type: 'liste', items: [
        '1. Activez les postes à renseigner dans Collecte.',
        '2. Exportez le fichier local collecte-gabarit.ocllm.json.',
        '3. Retirez ou anonymisez les éléments confidentiels, puis transmettez éventuellement le gabarit à un LLM hors de l’application.',
        '4. Demandez un retour JSON contenant uniquement la clé propositions, avec les posteId, facteurId et unités fournis.',
        '5. Importez le fichier JSON localement dans Open Carbon Tool.',
        '6. Relisez, corrigez, acceptez ou rejetez chaque ligne ; aucune ligne n’est ajoutée avant votre validation finale.',
        '7. Conservez les factures, relevés, exports comptables et autres justificatifs de chaque donnée acceptée.',
      ] },
      { type: 'encadre', ton: 'info', titre: 'Prompt de structuration', texte: 'Le prompt ci-dessous est prêt à copier. Il ne doit pas être exécuté par l’application : copiez-le dans le service choisi par l’utilisateur, avec le fichier gabarit.' },
      { type: 'prompt', titre: 'Prompt 1 — remplir le gabarit', texte: 'Tu es un assistant de structuration de données pour un bilan carbone.\nLis uniquement les postes actifs, les facteurIds et les unités fournis dans collecte-gabarit.ocllm.json. Ne crée aucun facteurId, n’invente aucune valeur et ne convertis aucune unité sans donnée source. Retourne uniquement un JSON valide avec une clé « propositions ». Pour chaque proposition, indique posteId, facteurId, valeur numérique, unité exacte, précision estimée, justification et source. Si une donnée manque, laisse-la absente et explique ce qui doit être demandé à l’utilisateur.' },
      { type: 'prompt', titre: 'Prompt 2 — analyser un bilan existant', texte: 'Analyse ce fichier .ocbilan.json comme un relecteur carbone. Respecte factorsVersion, les unités, les scopes et la distinction emission_t/amont_t/total_t. Ne recalcule pas avec un facteur externe et ne modifie pas le fichier. Retourne un diagnostic : données manquantes, facteurs ou sources à vérifier, postes dominants, incohérences possibles et questions à poser. Signale les incertitudes et ne présente aucune estimation inventée comme un fait.' },
      { type: 'encadre', ton: 'info', titre: 'Recommandation officielle du produit', texte: 'Pour analyser un bilan existant, donnez votre fichier .ocbilan.json à un LLM pour une analyse de votre bilan, plutôt qu’une capture d’écran ou un résumé incomplet. Ce fichier lisible et versionné porte les résultats, les facteurs utilisés et les sources nécessaires à une première analyse.' },
      { type: 'encadre', ton: 'alerte', titre: 'Limites méthodologiques et confidentialité', texte: 'Un LLM peut halluciner un facteur, une unité, une source ou un ordre de grandeur : aucune proposition n’est fiable sans revue humaine. P1 par défaut est une convention d’entrée, pas une preuve de précision. Vérifiez les unités, les facteurs, les sources et les ordres de grandeur ; ne transférez que les données autorisées, car le fichier peut contenir le nom de l’entreprise, des effectifs, des montants, des sites et des justificatifs. Le bilan reste un diagnostic et ne devient pas une mesure carbone automatique ni un BEGES réglementaire par l’usage d’un LLM.' },
      { type: 'p', texte: 'Les facteurs utilisés après acceptation restent ceux du catalogue local et leurs snapshots sont conservés dans le résultat calculé. Un facteur externe doit être créé, sourcé et validé dans le processus normal des facteurs personnalisés ; il n’est jamais créé implicitement par l’import.' },
    ],
  },
  {
    id: 'ressources',
